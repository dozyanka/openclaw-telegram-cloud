...
Write-Host '=== OpenClaw cloud repo verification ===' -ForegroundColor Cyan
...
# Scan text files for common personal/local-machine traces and exposed Telegram token shape.
...
Write-Host '[OK] No known personal paths/names/bot token patterns found.' -ForegroundColor Green

if (Get-Command git -ErrorAction SilentlyContinue) {
  Write-Host "[OK] Git: $(git --version)" -ForegroundColor Green
} else {
  Write-Warning 'Git is not installed. Install with: winget install --id Git.Git -e'
}

if (Get-Command gh -ErrorAction SilentlyContinue) {
  Write-Host "[OK] GitHub CLI: $((gh --version | Select-Object -First 1))" -ForegroundColor Green
} else {
  Write-Warning 'GitHub CLI is not installed. Install with: winget install --id GitHub.cli -e'
}
...
if (Get-Command docker -ErrorAction SilentlyContinue) {
  ...
  Write-Warning 'Docker is not installed. Local container test is optional; GitHub Actions will also build the image after push.'
}
